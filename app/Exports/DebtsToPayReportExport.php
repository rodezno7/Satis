...
class DebtsToPayReportExport implements WithEvents, WithTitle
{
    ...
    {
    	return __('report.debts_to_pay_report');
    }
    ...
    public function registerEvents(): array
    {
    	return [            
    		AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->columnWidth('A', 15); // reference
                ...
                $event->sheet->setCellValue('A3', mb_strtoupper(__('lang_v1.reference')));
                ...
                foreach($transactions as $t){
                    ...
                    if($id != $t->supplier_id){
                        $counter = $transactions->where('supplier_id', $t->supplier_id)->count();
                        ...
                        $event->sheet->setCellValue('A'. $row, $t->supplier_name);
                        $row ++;
                    }
                    ...
                    $event->sheet->setCellValue('A'. $row, $t->reference);
