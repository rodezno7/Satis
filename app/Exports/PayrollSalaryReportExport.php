    ...
    public function registerEvents(): array
    {
    	return [            
    		AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->columnWidth('F', 16); // extra hours
                $event->sheet->columnWidth('G', 13); // other income
                $event->sheet->columnWidth('H', 13); // total_income
                ...
                $event->sheet->columnWidth('M', 15); // total deductions
                $event->sheet->columnWidth('N', 12); // total to pay
                $event->sheet->setFormat('A7:N' . $items, \PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_TEXT);
                ...
                $event->sheet->getDelegate()->getStyle('A1:N1')->getFont()->setSize(15);
    			$event->sheet->mergeCells('A1:N1');
                $event->sheet->setCellValue('A1', mb_strtoupper($this->business->name));

                /** Report name */
                $event->sheet->horizontalAlign('A2:N2', \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                ...
                /** Type Payroll */
                $event->sheet->horizontalAlign('A3:N3', \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                $event->sheet->getDelegate()->getStyle('A3:N3')->getFont()->setBold(true);
                $event->sheet->getDelegate()->getStyle('A3:N3')->getFont()->setSize(13);
                ...
                $event->sheet->getDelegate()->getStyle('A4:N4')->getFont()->setSize(13);
                ...
                $event->sheet->getStyle('A'. $count.':N'. $count,  $event->sheet->getHighestRow())->getAlignment()->setWrapText(true);
                ...
                $event->sheet->setCellValue('F'.$count, mb_strtoupper(__('payroll.extra_hours')));
                $event->sheet->setCellValue('G'.$count, mb_strtoupper(__('payroll.other_income')));
                $event->sheet->setCellValue('H'.$count, mb_strtoupper(__('payroll.total_income')));
                ...
                $event->sheet->setCellValue('M'.$count, mb_strtoupper(__('payroll.total_deductions')));
                $event->sheet->setCellValue('N'.$count, mb_strtoupper(__('payroll.total_to_pay')));
                ...
                foreach($payrollDetails as $payrollDetail){
                    ...
                    $event->sheet->setCellValue('F'. $count, $this->moduleUtil->num_f($payrollDetail->extra_hours, $add_symbol = true, $precision = 2));
                    $event->sheet->setCellValue('G'. $count, $this->moduleUtil->num_f($payrollDetail->other_income, $add_symbol = true, $precision = 2));
                    $event->sheet->getDelegate()->getStyle('H'. $count)->getFont()->setBold(true);
                    $event->sheet->setCellValue('H'. $count, $this->moduleUtil->num_f($payrollDetail->total_income, $add_symbol = true, $precision = 2));
                    ...
                    $event->sheet->setCellValue('M'. $count, $this->moduleUtil->num_f($payrollDetail->total_deductions, $add_symbol = true, $precision = 2));
                    $event->sheet->getDelegate()->getStyle('N'. $count)->getFont()->setBold(true);
                    $event->sheet->setCellValue('N'. $count, $this->moduleUtil->num_f($payrollDetail->total_to_pay, $add_symbol = true, $precision = 2));

                    $regular_salary += $payrollDetail->regular_salary;
                    $extra_hours += $payrollDetail->extra_hours;
                ...
                $event->sheet->setCellValue('F'.$count, $this->moduleUtil->num_f($extra_hours, $add_symbol = true, $precision = 2));
                $event->sheet->setCellValue('G'.$count, $this->moduleUtil->num_f($other_income, $add_symbol = true, $precision = 2));
                $event->sheet->setCellValue('H'.$count, $this->moduleUtil->num_f($total_income, $add_symbol = true, $precision = 2));
                $event->sheet->setCellValue('I'.$count, $this->moduleUtil->num_f($isss, $add_symbol = true, $precision = 2));
                $event->sheet->setCellValue('J'.$count, $this->moduleUtil->num_f($afp, $add_symbol = true, $precision = 2));
                $event->sheet->setCellValue('K'.$count, $this->moduleUtil->num_f($rent, $add_symbol = true, $precision = 2));
                $event->sheet->setCellValue('L'.$count, $this->moduleUtil->num_f($other_deductions, $add_symbol = true, $precision = 2));
                $event->sheet->setCellValue('M'.$count, $this->moduleUtil->num_f($total_deductions, $add_symbol = true, $precision = 2));
                $event->sheet->setCellValue('N'.$count, $this->moduleUtil->num_f($total_to_pay, $add_symbol = true, $precision = 2));
            },
        ];
    }
}
