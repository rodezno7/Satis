...
<section class="content no-print">
	...
	<input type="hidden" id="flag-correlative" value="0">

	{!! Form::open(['url' => action('SellReturnController@store'), 'method' => 'post', 'id' => 'sell_return_form' ]) !!}
	{!! Form::hidden('transaction_id', $sell->id); !!}
	{!! Form::hidden('business_location_id', $sell->location->id) !!}
	<input name="sell_return_id" type="hidden" id="parent_id" value="{{ !empty($sell->return_parent->id) ? $sell->return_parent->id : '' }}">
	<div class="box box-solid">
		...
		<div class="box-body">
			<div class="row">
				<div class="col-sm-4">
					<strong>@lang('sale.invoice_no'):</strong> {{ $sell->correlative }} <br>
					<strong>@lang('messages.date'):</strong> {{@format_date($sell->transaction_date)}}
				</div>
				<div class="col-sm-4">
					<strong>@lang('contact.customer'):</strong> {{ $sell->customer->is_default ? $sell->customer_name : $sell->customer->name }} <br>
			...
			<div class="row">
				<div class="col-sm-3">
					<div class="form-group">
						{!! Form::label("document_type", __('document_type.document_type') . ":*") !!}
						{!! Form::select("document_type", $document_types, !empty($sell->return_parent->document_types_id) ? $sell->return_parent->document_types_id : null,
							["class" => "form-control select2", "id" => "document_type", "placeholder" => __("document_type.document_type"), "required"]) !!}
					</div>
				</div>
				<div class="col-sm-3">
					<div class="form-group">
						{!! Form::label('invoice_no', __('invoice.correlative').':*') !!}
						{!! Form::text('invoice_no', !empty($sell->return_parent->correlative) ? $sell->return_parent->correlative : null,
							['class' => 'form-control', 'id' => 'correlative']); !!}
					</div>
				</div>
				<div class="col-sm-3">
					<div class="form-group">
				              	...
				              	<td>
						            ...
						            data-msg-abs_digit="@lang('lang_v1.decimal_value_not_allowed')"
			              			...
			              			data-msg-max-value="@lang('validation.custom-messages.quantity_not_available', [
										'qty' => $sell_line->formatted_qty,
										'unit' => ! empty($sell_line->product->unit) ? $sell_line->product->unit->short_name : ''
									])">
						            <input name="products[{{$loop->index}}][unit_price]" type="hidden" class="unit_price" value="{{ $sell_line->unit_price }}">
