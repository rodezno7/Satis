...
<section class="content-header">
          ...
          <div class="row">
            <div class="col-sm-4">
              <div class="form-group">
                ...
                {!! Form::select('clasification', ['product' => 'Producto', 'kits' => 'Kits', 'service' => 'Servicio'], $product->clasification, ['class' => 'form-control select2']); !!}
              </div>
            </div>
            <div class="col-sm-4">
              <div class="form-group">
        ...
        <div class="row">
          <div id="divExcludeService2">
            ...
            <div class="col-sm-3">
              <div class="form-group">
                <label>
                  {!! Form::checkbox('check_dai', 1, $product->check_dai, ['class' => 'input-icheck', 'id' => 'check_dai']); !!}
  ...
  <div id="divExcludeService4" @if(!auth()->user()->can('product.view_cost')) class='hide' @endif style="display: none;">
    <div class="boxform_u box-solid_u">
      <div class="box-body">
        <div class="row">
          <div class="col-sm-3">
      ...
      <div class="row">
        ...
        <div class="col-sm-3">
          <div class="form-group">
            ...
            {!! Form::text('volume', $product->volume, ['id' => 'volume', 'class' => 'form-control input_number', 'placeholder' => __('product.volume')]); !!}
          </div>
        </div>
        <div class="col-sm-3">
          <div class="form-group">
            {!! Form::label('download_time',  __('product.download_time') . ':') !!}
            {!! Form::text('download_time', $product->download_time, ['id' => 'download_time', 'class' => 'form-control', 'placeholder' => 'hh:mm:ss']); !!}
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="col-sm-12">
          <div class="text-center">
            <div class="btn-group">
              ...
              <button id="update_n_edit_opening_stock" type="submit" @if(empty($product->enable_stock)) disabled="true" @endif id="opening_stock_button"  value="update_n_edit_opening_stock" class="btn bg-purple submit_product_form">@lang('lang_v1.update_n_edit_opening_stock')</button>
    ...
    $.get(route, function(res){
      $(res).each(function(key,value){
        ...
        if (count >= 0)
        {
          ...
          valor.push(contk);
          var fila = '<tr class="selected" id="filak'+contk+'" style="height: 10px"><td><button id="bitem'+contk+'" type="button" class="btn btn-danger btn-xs remove-item" onclick="deleteChildren('+contk+', '+variation_id+');"><i class="fa fa-times"></i></button></td><td><input type="hidden" name="product_ids[]" value="'+variation_id+'">'+name+'</td><td>'+sku+'</td><td>'+brand+'</td><td><input type="hidden" name="clas_product['+contk+']" value="'+value.clasification+'">'+unit_label+'</td><td><input type="hidden" name="price['+contk+']" id="price'+contk+'" value="'+price+'">'+price+'</td><td><input type="number" id="quantity'+contk+'" name="quantity['+contk+']" class="form-control form-control-sm" min="0.01" value="'+value.quantity+'" onchange="getTotalKit()" required></td></tr>';
          $("#listak").append(fila);
          contk++;
        }
      });
