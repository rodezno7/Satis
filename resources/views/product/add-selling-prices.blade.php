								...
								<table class="table table-condensed table-bordered table-th-green text-center table-striped">
									...
									<tbody>
										...
										<tr>
											...
											@endif
											<td><span class="display_currency" data-currency_symbol="true">{{ $variation->sell_price_inc_tax, 4 }}</span></td>
											@foreach($price_groups as $price_group)
											<td>
												{!! Form::text('group_prices[' . $price_group->id . '][' . $variation->id . ']', !empty($variation_prices[$variation->id][$price_group->id]) ? round($variation_prices[$variation->id][$price_group->id], 4) : 0, ['class' => 'form-control input_number input-sm'] ); !!}
											</td>
											@endforeach
										</tr>
										@endforeach
