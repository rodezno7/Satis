...
<div class="col-sm-12">
    ...
    <table class="table table-bordered add-product-price-table table-condensed" id="product_variation_form_part">
        ...
        <tbody>
            @if($action == 'add')
                @include('optics.product.partials.product_variation_row', ['row_index' => 0])
            @else

                @forelse ($product_variations as $product_variation)
                    @include('optics.product.partials.edit_product_variation_row', ['row_index' => $action == 'edit' ? $product_variation->id : $loop->index])
                @empty
                    @include('optics.product.partials.product_variation_row', ['row_index' => 0])
                @endforelse
