			...
			<div class="box box-success">
				...
				<div class="box-header with-border">
					...
					<span class="text-success">
						@if ($is_quote)
							{{ $transaction->quote_ref_no }}
						@else
							{{ $transaction->doc_name }} : {{ $transaction->correlative }}
						@endif
					</span>
				...
				{!! Form::hidden('cashier_id', $transaction->cashier_id, ['id' => 'cashier_id']) !!}
				{!! Form::hidden('warehouse_id', $transaction->warehouse_id, ['id' => 'warehouse_id']) !!}
				...
				<input type="hidden" value="{{ $is_quote }}" id="is_quote">
				...
				{{-- Document with tax exempt --}}
				<input type="hidden" id="doc_tax_exempt" value="{{ $doc_tax_exempt }}">
				...
				<div class="box-body">
					...
					<div class="row">
						...
						{{-- Allowed credit --}}
						<input type="hidden" id="default_allowed_credit" value="{{ ! empty($transaction->customer) ? $transaction->customer->allowed_credit : 0 }}">
						<input type="hidden" name="allowed_credit" id="allowed_credit" value="{{ ! empty($transaction->customer) ? $transaction->customer->allowed_credit : 0 }}">
							...
							<div class="form-group">
								<div class="input-group">
									...
									{!! Form::text('staff_note', $transaction->staff_note,
										['class' => 'form-control', 'id' => 'staff_note', 'placeholder' => __('accounting.comment'), 'style' => 'width: 100%']); !!}
								</div>
							</div>
						</div>
						...
						<div class="col-sm-3" @if ($pos_settings['show_order_number_field'] == 0) style="display: none;" @endif>
							<div class="form-group">
								<div class="input-group">
									...
									{!! Form::text('sale_note', $transaction->additional_notes,
										['class' => 'form-control', 'id' => 'sale_note', 'placeholder' => __('sale.no_order'), 'style' => 'width: 100%']); !!}
								</div>
							</div>
						</div>
					</div>
					...
					<div class="row col-sm-12 pos_product_div">
						...
						@php
							$hide_tax = '';
							if (session()->get('business.enable_inline_tax') == 0  && ! $is_admin){
								$hide_tax = 'hide';
							}
						@endphp
