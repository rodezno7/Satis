...
BEGIN
    ...
    SELECT
        ...
        ct.name AS city,
        (
            SELECT CONCAT(eo.first_name, ' ', eo.last_name)
            FROM lab_orders AS lo
            LEFT JOIN graduation_cards AS gc
                ON gc.id = lo.graduation_card_id
            LEFT JOIN employees AS eo
                ON eo.id = gc.optometrist
    ...
    WHERE t.business_id = v_business_id
        ...
        AND (
            v_commission_agent = 0 OR (t.commission_agent = v_commission_agent OR v_commission_agent = (
                SELECT uc.id
                FROM lab_orders AS loc
                LEFT JOIN graduation_cards AS gcc
                    ON gcc.id = loc.graduation_card_id
                LEFT JOIN employees AS eoc
                    ON eoc.id = gcc.optometrist
                LEFT JOIN users AS uc
                    ON uc.id = eoc.user_id
                WHERE loc.transaction_id = t.id
                LIMIT 1
            ))
        )
        ...
        AND (
            ...
            CONCAT(u.first_name, ' ', u.last_name) LIKE CONCAT('%', v_search, '%') OR
