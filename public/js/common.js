...
$(document).ready( function(){
	...
	jQuery.extend( $.fn.dataTable.defaults, {
        ...
        aLengthMenu: [
	        [5, 25, 50, 100, 200, 500, -1],
	        [5, 25, 50, 100, 200, 500, LANG.all]
	    ],
	    iDisplayLength: 25,
	    language: {
		    "search": LANG.search + ":",
...
// Check for number string in input field
$(document).on('keypress', 'input.input_only_number', function (event) {
	var regex = new RegExp(/^[0-9]+$/);
	var key = String.fromCharCode(!event.charCode ? event.which : event.charCode);
	if (!regex.test(key)) {
   		event.preventDefault();
...
// Check for number, +, - string in input field
$(document).on('keypress', 'input.input_graduation', function (event) {
	var regex = new RegExp(/^[0-9-+.]+$/);
	var key = String.fromCharCode(!event.charCode ? event.which : event.charCode);
	if (!regex.test(key)) {
		event.preventDefault();
...
// Check for number, +, - string in input field
$(document).on('keypress', 'input.input_prism', function (event) {
	var regex = new RegExp(/^([a-zA-Z0-9 .ñÑáÁéÉíÍóÓúÚüÜ]+)$/);
	var key = String.fromCharCode(!event.charCode ? event.which : event.charCode);
	if (!regex.test(key)) {
		event.preventDefault();
		return false;
...
// Allow 4 decimals only
$(document).on("change", "input.four_decimals", function(){
	var value = __read_number($(this));
	value = (value == undefined) ? 0 : value;
	__write_number($(this), value, false, 4);
});
