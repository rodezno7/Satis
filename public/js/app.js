$(document).ready(function() {
    ...
    $(document).on('submit', 'form#brand_add_form', function(e) {
        ...
        let formData = document.getElementById('brand_add_form');
    ...
    $(document).on('click', 'button.edit_brand_button', function() {
        ...
        $("div.brands_modal").load($(this).data('href'), function() {
            ...
            $('form#brand_edit_form').submit(function(e) {
                ...
                let formData = document.getElementById('brand_edit_form');
                let data = new FormData(formData);
    ...
    $('.contact_modal').on('shown.bs.modal', function(e) {
        $("input#nit").mask("0000-000000-000-0");
        $("input#nit_edit").mask("0000-000000-000-0");

        $("input#dni").mask("00000000-0");
        $("input#dni_edit").mask("00000000-0");
        ...
        $('select#is_exempt').on('change', function(){
            // $('select#tax_group_id')
            if($(this).val() == 1){
                $('select#tax_group_id').attr('disabled', true);
            }else{
                $('select#tax_group_id').attr('disabled', false);
            }
        });

        $('input#nit').on('change', function() {
            let nit = $(this).val();
            let route = '/contact/verify-if-exists-tax-number';
            if(nit != ""){
                $.ajax({
                    method: "get",
                    url: route,
                    ...
                    success: function(result) {
                        if (result.success) {
                            ...
                            Swal.fire({ title: result.msg, icon: "info", timer: 4000});
        ...
        $('input#nit_edit').on('change', function() {
            let nit = $(this).val();
            let contact_id = $('input#hidden_id').val();
            let route = '/contact/verify-if-exists-tax-number';
            if(nit != ""){
                $.ajax({
                    method: "get",
                    url: route,
                    data: {'id': contact_id,'nit': nit},
                    dataType: "json",
                    success: function(result) {
                        if (result.success) {
                            $('button#submit').attr('disabled', false);
        ...
        $('input#dni').on('change', function () {
            let dni = $(this).val();
            let route = '/contact/verify-if-exists-dni';
        ...
        // On change of dni_edit input
        $('input#dni_edit').on('change', function () {
            let dni = $(this).val();
            let contact_id = $('input#hidden_id').val();
            let route = '/contact/verify-if-exists-dni';
        ...
        $("form#contact_add_form, form#contact_edit_form").submit(function(e) {
            ...
            submitHandler: function (form) {
                ...
                if ($('input#is_supplier').is(":checked") || $('input#is_provider').is(":checked")) {
                    $.ajax({
                        method: "POST",
    ...
    //initialize iCheck
    $('input[type="checkbox"].form-check-input, input[type="radio"].input-icheck').iCheck({
        checkboxClass: 'icheckbox_flat-blue',
        radioClass: 'iradio_flat-blue'
    });

    $(document).on('ifChecked', '.check_all', function() {
        $(this).closest('.check_group').find('.form-check-input').each(function() {
            $(this).iCheck('check');
        });
    });
    ...
    // On change of payment_types_dropdown select
    $(document).on('change', '.payment_types_dropdown', function() {
        let payment_type = $(this).val();
        let input_id = $(this).attr('id');

        if (payment_type == 'card') {
            checkPosNumber(payment_type, input_id);
        } else {
            showPaymentRow(payment_type, input_id);
        }
    });
    ...
    //Annul Sale
	$(document).on('click', '.annul-sale', function(e){
		e.preventDefault();
		swal({
		  title: LANG.sure,
		  text: LANG.action_will_not_undo,
          icon: "warning",
          buttons: true,
          dangerMode: true,
        }).then((willDelete) => {
...
$(document).on('click', 'td.clickable_td_return', function(e) {
    e.preventDefault();
    e.stopPropagation();
    if ($(this).find('a').length) {
        var href = $(this).find('a').attr('href');
 ...
 * @return void 
 */
function checkPosNumber(payment_type, input_id) {
    let payment_types_dropdown = $('#' + input_id);

    $.ajax({
        method: 'post',
        url: '/pos/check-pos-number',
        dataType: 'json',
        success: function(result) {
            if (result.pos_number > 0) {
                showPaymentRow(payment_type, input_id);
            } else {
                payment_types_dropdown.val('cash').change();
                showPaymentRow('cash', input_id);
                Swal.fire({
                    title: result.title,
                    html: result.msg,
                    icon: 'error',
 ...
 * @return void 
 */
function showPaymentRow(payment_type, input_id) {
    let payment_types_dropdown = $('#' + input_id);
    let to_show = null;

    payment_types_dropdown.closest('.payment_row').find('.payment_details_div').each(function() {
