...
function __currency_convert_recursively(element, use_page_currency = false){
	...
	element.find('.display_currency').each(function(){
		...
		$(this).text(__currency_trans_from_en( value, show_symbol, use_page_currency, precision));
	});
}
...
function __sum_stock(table, class_name, label_direction = 'right'){
	...
	for (var key in stocks) {
		if (label_direction == 'left') {
			stock_html += key + ' : <span class="display_currency" data-currency_symbol="true">'
                    + stocks[key] + '</span> ' + '</br>';
        } else {
...
/**
 * Adds amounts from the column and returns the total.
 */
function __sum_column(table, class_name) {

	var total = 0;
 	table.find('tbody').find('tr').each( function(){
		element = $(this).find('.' + class_name);
		total += parseFloat(element.data('orig-value'));
	});
	
	var total_html = '<p class="text-left"><small><span class="display_currency" data-currency_symbol="false">' +
		total + '</span></small></p>';
